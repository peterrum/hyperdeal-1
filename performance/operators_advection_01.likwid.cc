     ...
     const std::string                   file_name)
{
...

  {
    timers.enter("apply");
    {
      ...
      for (unsigned int i = 0; i < param.n_iterations_warmup; i++)
        advection_operation.template apply<hyperdeal::advection::AdvectionOperationEvaluationLevel::all>(vec_dst, vec_src, 0.0);
    }
  ...
  if(param.details)
  {
    timers.enter("all1");
    {
      ...
      hyperdeal::ScopedTimerWrapper timer(timers, "total");
      for (unsigned int i = 0; i < param.n_iterations; i++)
        advection_operation.template apply<hyperdeal::advection::AdvectionOperationEvaluationLevel::cell>(vec_dst, vec_src, 0.0, &timers);
      timers.leave();
    }
    timers.leave();
  }
  ...
  if(param.details)
  {
    timers.enter("all2");
    {
      ...
      hyperdeal::ScopedTimerWrapper timer(timers, "total");
      for (unsigned int i = 0; i < param.n_iterations; i++)
        advection_operation.template apply<hyperdeal::advection::AdvectionOperationEvaluationLevel::all_without_neighbor_load>(vec_dst, vec_src, 0.0, &timers);
      timers.leave();
    }
    timers.leave();
  }
  ...
  if(param.details)
  {
    timers.enter("all3");
    {
      ...
      hyperdeal::ScopedTimerWrapper timer(timers, "total");
      for (unsigned int i = 0; i < param.n_iterations; i++)
        advection_operation.template apply<hyperdeal::advection::AdvectionOperationEvaluationLevel::all>(vec_dst, vec_src, 0.0, &timers);
      timers.leave();
    }
    timers.leave();
  }
  ...
  table.set("throughput without timers [GDoFs/s]",
            matrixfree_wrapper.n_dofs() * param.n_iterations /
              timers["apply:withouttimers:total"].get_accumulated_time() / 1000);
