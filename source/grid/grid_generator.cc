...
public:
  DeformedCubeManifold(const dealii::Point<dim>       left,
    ...
    : left(left[0]), right(right[0]), deformation(deformation), frequency(frequency)
  {
        ...
        AssertThrow(check(left), dealii::StandardExceptions::ExcInternalError());
        AssertThrow(check(right), dealii::StandardExceptions::ExcInternalError());
  }
  ...
  pull_back(const dealii::Point<dim> & space_point) const
  {
    ...
    AssertThrow(residual.norm() < 1e-12, dealii::StandardExceptions::ExcMessage("Newton for point did not converge."));
    return x;
  }
              ...
              &*tria_x))
        {
          if (auto triangulation_v = dynamic_cast<
                ...
                &*tria_v))
            {
              ...
              {
                static internal::DeformedCubeManifold<dim_x> manifold(left_x, right_x);
